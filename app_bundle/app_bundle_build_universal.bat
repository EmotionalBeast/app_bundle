...
REM build universal .apks from aab file
REM
...
echo.
echo start build universal .apks from *.aab files, please wait...
echo.
...
for  %%f in (%cd%\%workspace%\*.aab) do (
	...
	set output_apks=!out_path!-universal.apks
	if exist !output_apks! (del !output_apks!)

	java -jar %bundletooljar% build-apks --mode=universal --bundle=!target_aab! --output=!output_apks! --ks=%ks_file% --ks-pass=%ks_pass%  --ks-key-alias=%ks_alias% --key-pass=%ks_pass%
